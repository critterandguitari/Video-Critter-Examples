...
unsigned int soundBufR = 0;         // index when reading buffer
unsigned int soundBufW = 0;         // index when writing to buffer
...
{ 
    ...
    DACR = soundBuf[soundBufR];         // spit out the audio to the DAC
    soundBufR++;
    soundBufR &= 0x1ff;
    ...
    //imageVindex >>= 1;                // it is 2 lines on the TV, uncomment for 100 lines

    imageVindex = lineCount - 25;       // this for 120 lines
    imageVindex *= 6;                   // 
    imageVindex /= 10;
